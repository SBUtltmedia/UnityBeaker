...
public class OldParticleGenerator : MonoBehaviour {		
	...
	public OldDynamicParticle.STATES particlesState= OldDynamicParticle.STATES.WATER; // The state of the particles spawned
	...
	void Update() {	
		if( lastSpawnTime+SPAWN_INTERVAL<Time.time ){ // Is it time already for spawning a new particle?
			GameObject newLiquidParticle=(GameObject)Instantiate(Resources.Load(ResourceDirectories.DefaultDynamicParticle)); //Spawn a particle
			newLiquidParticle.GetComponent<Rigidbody2D>().AddForce( particleForce); //Add our custom force
            OldDynamicParticle particleScript =newLiquidParticle.GetComponent<OldDynamicParticle>(); // Get the particle script
